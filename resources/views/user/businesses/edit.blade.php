...
<head>
    ...
    <title>Edit Usaha - Pemilik Usaha</title>
        ...
        <div class="flex-1 p-4 lg:p-8 w-full">
            <div class="bg-white rounded-2xl lg:rounded-3xl shadow-2xl p-4 lg:p-8 border border-gray-100">
                <div class="mb-6">
                    ...
                    <h2 class="font-serif text-2xl lg:text-4xl font-bold text-[#1a3a2b]">Edit Usaha</h2>
                    <p class="mt-2 text-sm text-gray-600">Update data usaha kamu</p>
                </div>
                ...
                {{-- ═══════════════════════════════════════════
                    FORM UPDATE USAHA (GALERI DI DALAM)
                    ═══════════════════════════════════════════ --}}
                <form action="{{ route('user.businesses.update', $business->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="name" id="businessSelect" class="w-full border-2 border-gray-300 rounded-lg p-2 focus:outline-none focus:border-[#1a3a2b]" required></select>
                            </div>

                            <div class="mb-4">
                                <label class="block text-sm font-medium mb-2">Jenis / Tipe</label>
                                <input type="text" name="type" value="{{ $business->type }}" placeholder="Contoh: Kos Putra A" class="w-full border-2 border-gray-300 rounded-lg p-2 focus:outline-none focus:border-[#1a3a2b]">
                            </div>

                            <div class="mb-4">
                                <label class="block text-sm font-medium mb-2">Deskripsi</label>
                                <textarea name="description" rows="3" class="w-full border-2 border-gray-300 rounded-lg p-2 focus:outline-none focus:border-[#1a3a2b]">{{ $business->description }}</textarea>
                            </div>

                            <div class="mb-4">
                                <label class="block text-sm font-medium mb-2">Alamat</label>
                                <input type="text" name="address" value="{{ $business->address }}" placeholder="RT 04 Ngijo, Karangploso, Malang" class="w-full border-2 border-gray-300 rounded-lg p-2 focus:outline-none focus:border-[#1a3a2b]">
                            </div>

                            <div class="mb-4">
                                <label class="block text-sm font-medium mb-2">Link Google Maps</label>
                                <input type="text" name="google_maps_link" id="mapsLinkInput" value="{{ $business->google_maps_link ?? '' }}" placeholder="https://www.google.com/maps/place/..." class="w-full border-2 border-gray-300 rounded-lg p-2 focus:outline-none focus:border-[#1a3a2b]">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="phone" value="{{ $business->phone }}" placeholder="6281234567890" class="w-full border-2 border-gray-300 rounded-lg p-2 focus:outline-none focus:border-[#1a3a2b]">
                            </div>

                            <div class="mb-4">
                                <label class="block text-sm font-medium mb-2">Fasilitas / Menu</label>
                                <input type="text" name="facilities" value="{{ $business->facilities }}" placeholder="Fasilitas 1, Fasilitas 2" class="w-full border-2 border-gray-300 rounded-lg p-2 focus:outline-none focus:border-[#1a3a2b]">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {{-- ⭐ GALERI DETAIL — DI DALAM FORM UPDATE, DEKET FOTO --}}
                ...
                {{-- ═══════════════════════════════════════════
                    ...
                    (Hidden, connect lewat form="" attribute)
                    ═══════════════════════════════════════════ --}}
                @foreach ($galeriImages as $index => $img)
                <form id="removeImageForm{{ $index }}"
                    action="{{ route('user.businesses.remove-image', $business->id) }}"
                    method="POST"
                    style="display:none;">
                    @csrf
                    @method('DELETE')
